...
hnum = 4;   %number of hospitals

h1.x = 1;   %x coordinate
h1.y = 15;   %y coordinate
h1.req = 1;     %number of medipacks required

h2.x = 7;   %x coordinate
h2.y = 19;   %y coordinate
h2.req = 1;     %number of medipacks required

h3.x = 4;   %x coordinate
h3.y = 10;   %y coordinate
h3.req = 2;     %number of medipacks required

h4.x = 17;   %x coordinate
h4.y = 5;   %y coordinate
h4.req = 1;     %number of medipacks required

all_h = [h1 h2 h3 h4];

%% initialize drones
dronenum = 2;   %number of different drone choices

drone1.packs = 2;    %number of packs drone is capable of holding at a time
drone1.radius = 2;  %radius with which the drone can survey roads it passes
drone1.maxv = 5;   %max drone speed

drone2.packs = 1;    %number of packs drone is capable of holding at a time
drone2.radius = 2;  %radius with which the drone can survey roads it passes
drone2.maxv = 11;   %max drone speed

all_drone = [drone1 drone2];
...
all_cost = [];
    for path=[1:length(sorted_paths)]
        for config=[1:length(sorted_paths_drones(1,1,:))]
                all_cost= [all_cost cost_fnc(sorted_paths(path,:), sorted_paths_drones(path,:,config), all_paths_lines(i,path,:,:), all_drone, dronenum)];
        end
    end
